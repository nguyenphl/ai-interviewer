...
export async function getDashboardData(): Promise<DashboardStats> {
  ...
  const avgScore =
    recentSessions.length > 0
      ? parseFloat(
          (recentSessions.map((s) => s.totalScore ?? 0).reduce((a, b) => a + b, 0) / recentSessions.length).toFixed(2)
        )
      : 0;
  ...
  const activityDates = Array.from(activityMap.entries()).map(([date, count]) => ({ date, count }));
  ...
  return {
    ...
    avgScore,
    streak,
    topicStats: topicStats.map((t) => ({ ...t, lastSeenAt: t.lastSeenAt.toISOString() })),
